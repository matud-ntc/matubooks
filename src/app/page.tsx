...
export default function HomePage() {
  ...
  const allStyles = Array.from(
    new Set(books.flatMap((book) => book.style || []))
  );
  ...
  const filtered = books.filter((book) => {
    ...
    const matchesStyle =
      selectedStyle === 'Todos' ||
      (book.style && book.style.includes(selectedStyle));
    ...
    <main className="min-h-screen bg-[#f2ebe1] px-6 py-10">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col gap-6 mb-12">
          <h1 className="text-5xl font-serif font-bold text-center">Biblioteca</h1>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-4">
              <select
                value={sortBy}
                onChange={(e) => handleSortChange(e.target.value as 'author' | 'title')}
